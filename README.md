...
| **`rocq_start`** | Start an interactive proof session and return proof goals. Three modes: (1) by theorem name, (2) by position — jump to any point in a file to inspect proof goals there (e.g., error positions from `rocq_compile`); cursor rounds forward through the sentence containing it, so a cursor anywhere on a sentence (including its period) yields the state **after** that sentence, and whitespace before a sentence yields the state **before** it (see docstring for the full rule), (3) from imports. Returns a `state_id` for use with `rocq_check` and `rocq_step_multi`. Optional `where="before"` reports the state *before* the sentence beginning on `line` without you computing the whitespace column, and an empty `goals` response carries `resolved` (the position actually used, that line's text, and which side was asked for). A cold call that outruns `ROCQ_SOFT_DEADLINE` returns `status: "pending"` with a `job_id` rather than being killed by the client — see the **Long first calls** callout. Optional `force_restart=True` kills pet and clears the state table — recovery primitive for accumulated RAM bloat, indexing corruption, or a state expiry that repeats after a plain retry (see Concurrency model). |
...
| Tool | Description |
|------|-------------|
| **`rocq_diag`** | Operational diagnostics: pet health, memory headroom, system load average, recent errors, currently-live proof states. Use after `pet_restarted: True` to diagnose what happened, before a long `vm_compute` to check memory headroom, or **as an orchestrator's monitoring primitive** — call it between sub-agent dispatches to spot shared-pet contention (`live_states[*].file` shows entries from peer callers), accumulating RAM bloat, or a pile-up in `recent_errors`. Also returns `jobs` — the background work handed off past the client's per-call deadline; a `"running"` entry there is the signal to `rocq_poll` rather than to re-issue the call or fall back to a whole-file build. Does not spawn pet if it is not running; safe to call without `pet` installed. |
...
| **`rocq_poll`** | Collect the result of a call handed off to the background. `rocq_start` / `rocq_compile_file` return `status: "pending"` with a `job_id` when the work outruns `ROCQ_SOFT_DEADLINE`; `rocq_poll(job_id, wait=<seconds>)` returns the finished result, a fresh `pending` envelope, or `reason: "not_found"` once the result has expired. A `pending` envelope is **not** a failure — the work is still running. See the **Long first calls** callout. |
...
> **Long first calls (`ROCQ_SOFT_DEADLINE`):** MCP clients impose their own per-call deadline — 60s in the clients measured — *above* this server. Neither the `timeout=` argument nor `ROCQ_PET_TIMEOUT` can move it, so on a large development the first `rocq_start` / `rocq_compile_file` against a file, which has to replay that file's prefix, was reliably killed mid-flight. Since `ROCQ_SOFT_DEADLINE` (default 20s, below that client deadline) those tools now hand the work to a background job instead and return `{"success": false, "status": "pending", "reason": "warming", "job_id": ..., "retry_after_s": ...}`. **The work continues.** Collect it with `rocq_poll(job_id, wait=...)`, or re-issue the identical call — it attaches to the same job rather than starting a second replay behind the first on the pet lock. A key covers the tool, its arguments, the workspace, and the file's mtime, so an edited file never attaches to a job started against its previous contents. Never answer a `pending` envelope with a whole-file build: it discards work that is still running. Set `ROCQ_SOFT_DEADLINE=0` to restore the old await-to-completion behaviour.
>
> **`ROCQ_SOFT_DEADLINE` is not what bounds a job's life.** It decides only when the *caller* stops waiting; the work is killed by that tool's own timeout, which differs between the two:
>
> - `rocq_start` is pet-routed: the per-call `timeout=`, else `ROCQ_PET_TIMEOUT` (default **30**), clamped to `ROCQ_QUERY_TIMEOUT_CAP` (default 300). Expiry kills pet (`reason: "timeout"`, `pet_restarted: True`).
> - `rocq_compile_file` is coqc-routed: the per-call `timeout=`, else `ROCQ_COQC_TIMEOUT` (default **60**), **not** subject to `ROCQ_QUERY_TIMEOUT_CAP` — coqc-routed tools honour any `timeout=` you pass.
>
> Two consequences. **The soft deadline must stay below whichever of those applies**, or the work dies before the handoff can happen and you never see `pending`; the shipped defaults (20 against 30 and 60) are ordered correctly, and the server emits a `RuntimeWarning` at startup if `ROCQ_PET_TIMEOUT` is reconfigured below the soft deadline. **A job outlives the client's deadline but not its tool's timeout**, so the stock defaults buy a `pending` envelope at 20s and then 30s (or 60s) of total work — for a cold replay that genuinely takes minutes, pass `timeout=` sized to the whole replay, and for `rocq_start` raise `ROCQ_QUERY_TIMEOUT_CAP` to go beyond its 300s ceiling.
...
- **Validation / lookup** (set by tools before reaching `pet`): `"validation"`, `"not_found"` (typo on `rocq_start` / `rocq_assumptions`; also an unknown or expired `job_id` on `rocq_poll`, whose response then carries `live_jobs`).
- **Handoff, not a failure**: `"warming"` on `rocq_start` / `rocq_compile_file` / `rocq_poll`, always paired with `status: "pending"` and a `job_id`. `success` is `False` because there is no result *yet* — the work is still running, and the only correct responses are `rocq_poll` or re-issuing the identical call. Never treat it as a failure and never fall back to a whole-file build; see the **Long first calls** callout.
...
| `ROCQ_PET_TIMEOUT` | `30` | Timeout (seconds) for pytanque-based tools, and so the ceiling on a handed-off `rocq_start` (a handed-off `rocq_compile_file` is bounded by `ROCQ_COQC_TIMEOUT` instead). Must stay **above** `ROCQ_SOFT_DEADLINE` (the default pair, 30 and 20, is ordered correctly) or the background handoff is unreachable — see the **Long first calls** callout; the server warns at startup otherwise. Not itself subject to `ROCQ_QUERY_TIMEOUT_CAP`, which clamps only an explicitly passed `timeout=` |
| `ROCQ_PET_TIMEOUT_GRACE` | `10` | Seconds added to the effective timeout of `rocq_check` / `rocq_step_multi` to get the process-level hard timeout. The inner budget is enforced by Rocq's own `Timeout` command, whose expiry is a recoverable `tactic_failed` that leaves pet alive; the grace gives it room to fire before the hard kill. Raise it only if heavy tactics are being hard-killed where Rocq should have timed out first |
| `ROCQ_QUERY_TIMEOUT_CAP` | `300` | Cap (seconds) on the per-call `timeout` parameter of any pytanque-based tool (`rocq_query`, `rocq_start`, `rocq_step_multi`, `rocq_check`, `rocq_assumptions`, `rocq_toc`, `rocq_notations`); larger values are clamped and the response carries `clamped_timeout: <cap>`. Bounds how long one caller can park the process-global pet lock, so it applies only to an explicitly passed `timeout=`: it does not clamp `ROCQ_PET_TIMEOUT` (the fallback when none is passed — the server warns at startup if that exceeds the cap), and the coqc-routed tools (`rocq_compile`, `rocq_compile_file`, `rocq_verify`) are not subject to it at all |
| `ROCQ_SOFT_DEADLINE` | `20` | Seconds a `rocq_start` / `rocq_compile_file` caller waits before the work is handed to a background job and a `status: "pending"` envelope is returned. Must stay below the MCP client's own per-call deadline (typically 60s) and below the timeout of the tool being handed off (`ROCQ_PET_TIMEOUT` for `rocq_start`, `ROCQ_COQC_TIMEOUT` for `rocq_compile_file`, or either one's per-call `timeout=`), which is what actually bounds a job's life. `0` disables the handoff and restores await-to-completion. |
| `ROCQ_JOB_RETENTION` | `900` | Seconds a finished job's result stays collectable by `rocq_poll` before it is evicted. |
| `ROCQ_MAX_JOBS` | `64` | Cap on tracked jobs. Finished jobs are evicted oldest-first; running jobs are never evicted. |
...
src/rocq_mcp/
  ...
  server.py              MCP server, 14 @mcp.tool wrappers, pet subprocess management
  ...
  jobs.py                Background job registry: keeps a cold call alive past the client's deadline
...
tests/
  ...
  test_jobs.py          Tests for the background job registry / rocq_poll
  test_where.py         Tests for where= position resolution and the resolved report
